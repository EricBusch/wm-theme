				...
				<a href="<?php echo get_permalink( $collection->ID ); ?>"
				   class="bg-white shadow rounded overflow-hidden hover:shadow-xl transition-shadow duration-300">
					...
					<p class="px-6 mb-6 text-sm text-grey-600">
						Learn to write <?php esc_html_e($word_count); ?> <?php esc_html_e($plural_title); ?> with these <?php esc_html_e($page_count); ?> printable PDF worksheets.
					</p>
					<div class="">
						<?php echo get_the_post_thumbnail( $collection->ID, 'wm-collection-wkst', [ 'class' => 'object-cover h-48 w-full' ] ); ?>
					</div>
