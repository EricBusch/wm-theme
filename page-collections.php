		...
		<div class="mx-auto max-w-7xl px-6">
			<div class="py-24 max-w-4xl mx-auto flex flex-col">
				...
				<h1 class="text-6xl text-balance text-center font-black tracking-tighter py-2">
					Fun activities to <span
						class="bg-clip-text text-transparent bg-gradient-to-r from-secondary-500 to-pink-500">master writing Chinese</span>
				</h1>
				<p class="text-xl text-balance text-center mt-6 text-grey-700 leading-relaxed">
					Over 150 collections of <strong>free printable Chinese writing worksheets</strong> to help you read
					& write
					Chinese faster than you thought possible!
				</p>
			</div>
...
<?php $groups = wm_get_all_collection_groups(); ?>

	<div class="grid md:grid-cols-3 gap-6 mx-auto max-w-7xl mb-12 mt-12 px-6">
		<?php foreach ( $groups as $group ) : ?>
			<a href="<?php echo get_term_link( $group->term_id ); ?>"
			   class="bg-white shadow rounded overflow-hidden hover:shadow-xl transition-shadow duration-300">
				...
				<div class="flex flex-col p-10 gap-y-4">
					<h2 class="text-primary-950 text-xl font-bold text-balance"><?php esc_html_e( $group->name ); ?></h2>
					<p class="flex flex-row items-center text-lg text-grey-600 gap-x-2">
							<span
								class="bg-grey-100 text-grey-600 w-10 h-10 flex flex-row items-center justify-center"><?php esc_html_e( $group->count ); ?></span>
						<span>Worksheet <?php printf( _n( 'Collection', 'Collections', $group->count, 'wm' ), $group->count ); ?></span>
					</p>
				</div>
			</a>
		<?php endforeach; ?>
